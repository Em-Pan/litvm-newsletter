...
const posts = {
  newsletterBanner: 'may-newsletter-banner-v2.jpg',
...
function metricCell(value, label, sublabel, accent = '#6bd7ff', last = false) {
  return `<td class="stat-col" align="left" valign="top" width="33.33%" style="width:33.33%; padding:18px 14px 17px 14px; ${last ? '' : 'border-right:1px solid rgba(107,215,255,.22);'}">
              <div style="font-family: Arial, Helvetica, sans-serif; font-size:10px; line-height:13px; color:${accent}; font-weight:800; letter-spacing:.12em; text-transform:uppercase; margin:0 0 7px 0;">${escapeHtml(label)}</div>
              <div style="font-family: Arial, Helvetica, sans-serif; font-size:34px; line-height:35px; color:#ffffff; font-weight:900; letter-spacing:-1.2px; margin:0;">${escapeHtml(value)}</div>
              <div style="font-family: Arial, Helvetica, sans-serif; font-size:11px; line-height:15px; color:#b8d7e1; font-weight:700; margin:7px 0 0 0;">${escapeHtml(sublabel)}</div>
            </td>`;
}
...
function statBand() {
  return `
          <tr>
            <td class="mobile-pad" style="padding: 2px 40px 30px 40px; background-color: #ffffff;">
              <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="width:100%; background-color:#071c25; border-radius:14px; overflow:hidden; border:1px solid #2e7892;">
                <tr>
                  <td colspan="3" style="padding:14px 16px 0 16px;">
                    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
                      <tr>
                        <td align="left" style="font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:15px;color:#6bd7ff;font-weight:900;letter-spacing:.13em;text-transform:uppercase;">May Network Snapshot</td>
                        <td align="right" style="font-family:Arial,Helvetica,sans-serif;font-size:10px;line-height:15px;color:#8fb7c5;font-weight:700;letter-spacing:.08em;text-transform:uppercase;">LiteForge in numbers</td>
                      </tr>
                    </table>
                  </td>
                </tr>
                <tr>
                  ${metricCell('3M+', 'Wallets', 'active wallets crossed', '#6bd7ff')}
                  ${metricCell('50M+', 'Throughput', 'transactions and climbing', '#9ff3c8')}
                  ${metricCell('$2K', 'Builder pool', 'LTC hackathon rewards', '#c8f16c', true)}
                </tr>
                <tr>
                  <td colspan="3" style="padding:0 16px 15px 16px;">
                    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="width:100%;">
                      <tr>
                        <td style="height:7px;background-color:#2a5664;border-radius:10px;overflow:hidden;"><div style="width:86%;height:7px;background-color:#6bd7ff;line-height:7px;font-size:0;">&nbsp;</div></td>
                      </tr>
                      <tr>
                        <td style="font-family:Arial,Helvetica,sans-serif;font-size:11px;line-height:16px;color:#b8d7e1;padding-top:8px;">A compact read on May: wallets, throughput, and builder incentives all moved in the same direction.</td>
                      </tr>
                    </table>
                  </td>
                </tr>
...
writeFileSync('LitVM_May_2026_v2_TEST.html', html('local'));
writeFileSync('LitVM_May_2026_v2_SENDGRID_READY.html', html('cdn'));
console.log('Wrote LitVM_May_2026_v2_TEST.html');
console.log('Wrote LitVM_May_2026_v2_SENDGRID_READY.html');
